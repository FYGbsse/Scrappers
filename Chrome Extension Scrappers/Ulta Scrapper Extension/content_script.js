function clickPlanet() {
    ...
    if (location.href.indexOf('https://www.ulta.com/') > -1) {
        alert('I am in www.ulta.com');
        getRewiews('https://www.ulta.com/p/blended-face-powder-xlsImpprod10791891');
        if(jQuery('.DesktopLeftNavOverlay_BodyScroll--open').length == 1){
            alert("i am in open nav");
            let time = 500;
            jQuery('.DesktopLeftNav__Header:last-child .DesktopLeftNav__navItemContainer  div.DesktopLeftNavSection .DesktopLeftNavSection__wrapper div .DesktopLeftNavSection__navItem:first-child').each(function(){
                let box = jQuery(this);
                setTimeout( function(){ 
                  let category_link = jQuery(box).find('a').attr('href');
                  console.log(category_link);
                   /* Sending ajax to category page */
                    $.ajax({url:'https://www.ulta.com/makeup-face?N=26y3',type:'get',async: true}).done(function(categorydata){
                        let product_list=$(categorydata);
                        let product_title= $(product_list).find('h1.breadcrumb-heading').text();
                      /*   console.log(product_title); */
                        let product_url_list = $(product_list).find('#product-category-cont ul li .productQvContainer .quick-view-prod > a')
                            /* each loop in category page to get the links of all products */
                            let time_2 = 500;
                        jQuery(product_url_list).each(function(){
                                let box_2 = jQuery(this);
                            setTimeout( function(){ 
                                let product_url =jQuery(box_2).attr('href');
                                console.log(product_url);
                                /* appending url as https://www.ulta.com/ + product_url */
                                let full_product_url = 'https://www.ulta.com'+ product_url;
                                /* sending ajax using producl_url to product page */
                                console.log(full_product_url);
                                jQuery.ajax({url:full_product_url,type:'get',cache:false,async:true}).done(function(pagedata){
                                let product_page=jQuery(pagedata);
                                let product_Name= jQuery(product_page).find('.ProductMainSection__productName span').text();
                                console.log(product_Name);
                                /* each loop to get the review */
                                let review_section= jQuery(product_page).find('#pr-review-display .pr-review');
                                jQuery(review_section).each(function(){
                                    let box_3 = jQuery(this);
                                    /* review data */
                                    let review_author= $(box_3).find('.pr-rd-author-nickname span span:last-child').text();
                                    let author_location= $(box_3).find('.pr-rd-author-location span span:last-child').text();
                                    let author_status= $(box_3).find('.pr-rd-badging-text').text();
                                    let review_date= $(box_3).find('.pr-rd-author-submission-date time').attr('datetime');
                                    let review_title= $(box_3).find('h5.pr-rd-review-headline').text();
                                    let review_body= $(box_3).find('.pr-rd-description-text').text();
                                    let review_rating= $(box_3).find('.pr-review .pr-snippet-rating-decimal');
                                    console.log('this is review_author' + review_author);
                                    console.log('this is author_location' + author_location);
                                    console.log('this is review title' + review_title);
                                    /* Creating csv */
                                    var csv = {};
                                        csv["product_title"] = product_Name;
                                        csv["product_url"] = full_product_url;
                                        csv["review_author"] = review_author;
                                        csv["author_location"] = author_location;
                                        csv["author_status"] = author_status;
                                        csv["review_date"] = review_date;
                                        csv["review_title"] = review_title;
                                        csv["review_body"] = review_body;
                                        csv["review_rating"] = review_rating;   
                                    console.log(csv);      
                                    /* putting in array and convert to json */
                                ...
                                /* each loop to get the review- END*/
                                });
                            }, time_2)
                            time_2 += 500;
                        });
...
function getRewiews(url){
    setTimeout(function(){    
    $.ajax({url:url ,type:'get',async: true}).done(function(producthtml){
        console.log('Here is rendered js html '+ producthtml);
        let review_data=jQuery(producthtml);
        let htmlk = jQuery(review_data).find('noscript').html();
        let h3= jQuery(review_data).find('.ProductMainSection__productName span').text();
        console.log("product name is 1 :" + htmlk);
        let product_Name= jQuery(review_data).find('.ProductMainSection__productName span').text();
        console.log("product name is 1 :" + product_Name);
        /* let el_index = 7;
        let p_index = 7;
        while(el_index <=31){
            let review_title= jQuery(html).find('h3').eq(el_index).text();
            let review_rating= jQuery(html).find('p').eq(p_index).text();
            let author_status= jQuery(html).find('p').eq(el_index + 1).text();
            let review_body= jQuery(html).find('p').eq(el_index + 2).text();
            console.log('this is heading' + review_title);
            console.log('this is author_rating' + review_rating);
            console.log('this is review status' + author_status);
            console.log('this is review review_body' + review_body);
            el_index ++;
            p_index = p_index + 3;
        } */
    });
