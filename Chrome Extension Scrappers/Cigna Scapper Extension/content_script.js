...
function clickPlanet() {
    ...
    if (location.href.indexOf('https://hcpdirectory.cigna.com/web/public/consumer/directory/dentists?') > -1) {
        alert('I am in Cigna');
...
async  function ReviewScrapper(){
   ...
   var currentPage =  1; //parseInt(jQuery('.ml-current-page span').text());
   ...
   while (hasNextPage) {
       console.log(`Scraping page ${currentPage}...`);
        ...
        const cards = jQuery('cigna-listings.ng-star-inserted');
        ...
        for (let i = 0; i < cards.length; i++) {
            ...
            const main = current.find('a.provider-name');
            const main_link = current.find('a.provider-name').attr('href');
            const tracker = jQuery('li.page-count').text().trim()+'--' +[i];
            var distance = current.find('[data-test-id="listings-distance-mileage"]').text().trim();
            console.log(`cards.main_link ${ main_link} ...`);
            ...
            var element = jQuery('cigna-listings.ng-star-inserted a.provider-name')[i];
            ...
            const getGmail = jQuery('[data-test-id="text-digital-contact-email"]').first().text().trim();
            const email = getGmail === '' ? 'nil' : getGmail;
            const website = window.location.href;
            const NPI = jQuery('[data-test-id="text-national-provider-id"]').text().trim();
            const id = parseInt(zipCode.toString() + NPI.toString(), 10); 
            const name1 = jQuery('[data-test-id="header-provider-name"]').text().trim();
    ...
    /*
            ...
            window.history.back();console.log('back');
        ...
        const nextPageButton = jQuery('[data-test="button-last-page"][disabled=""]'); // Update with the correct selector for the next page button
        if (nextPageButton.length > 0) {
            ...
            var element2 = jQuery('[data-test-id="button-next-page"]')[0];
