...
function clickPlanet() {
    ...
    if (location.href.indexOf('https://www.fragranticarabia.com/') > -1) {
        alert('I am in fragranticarabia');
        var male = 10; /* 12321; */
        var female = 10; /* 29316; */
        var both =10;  /* 26361; */
...
function ReviewScrapper(){
    chrome.storage.local.get(['manual_scrap_user_id'], function(result) {
    user_id = result.manual_scrap_user_id;
    var count = 9000;
    jQuery('.perfumes-row .cell.card').each(function(){
        
        var current = jQuery(this);   
        var search_url = jQuery(this).find('.card-section p a').attr('href');
        
        setTimeout(function(){
        console.log('Here is a product url '+search_url);
        $.ajax({url:search_url,type:'get',async:true}).done(function(productPageData){
            console.log(productPageData);
            let getReviewData=$(productPageData);
            let product_title= $(getReviewData).find('#toptop h1').text();
            let review_rating = $(getReviewData).find('div[itemprop="aggregateRating"] span[itemprop="ratingValue"]').text();
            let reviewCard = $(getReviewData).find('#all-reviews div.fragrance-review-box');
            jQuery(reviewCard).each(function(){
                let review_date = jQuery(this).find('span.vote-button-legend').text();
                let review_author = jQuery(this).find('b.idLinkify').text();
                let review_body = jQuery(this).find('div[itemprop="reviewBody"]').text();
                let review_image = jQuery(this).find('img.comments-avatar').attr('src');
                //creating array to store data 
                var csv = {};
                csv["product_title"] = product_title;
                csv["product_url"] = search_url;
                csv["review_author"] = review_author;
                csv["review_date"] = review_date;
                csv["review_body"] = review_body;
                csv["review_image"] = review_image;
        ...
        $(current).remove(); 
        if($('.perfumes-row .cell.card').length==0){
            console.log('i am going to click a button');
            $('.ais-InfiniteHits .button').click();
            setTimeout(function(){
            ReviewScrapper();
            /*  alert("check and scrape"); */
        },9000);
        }
        },count);
        /* if(count > 5){ 
