...
function clickPlanet() {
    ...
    if (location.href.indexOf('https://providers.online.metlife.com/findDentist') > -1 ||
      location.href.indexOf('https://www.metlife.com/') > -1){
        alert('I am in MetLife');
        var male = 10; /* 12321; */
        var female = 10; /* 29316; */
        var both =10;  /* 26361; */
       
       /*  if(window.location.href == "https://www.fragranticarabia.com/search/?spol=male"){
         ReviewScrapper(); 
        }
        else if(window.location.href == "https://www.fragranticarabia.com/search/?spol=female"){
            alert('female');
            ReviewScrapper(); 
        }
        else if(window.location.href == "https://www.fragranticarabia.com/search/?spol=unisex"){
            alert('both');
            ReviewScrapper(); 
        } */
        if (window.location.href.indexOf("findDentist") > -1 || window.location.href.indexOf("metlife") > -1) {
            alert("on search page");
            ReviewScrapper()
          }
        else{
            alert('Sorry i could not find any url');
        }
    }
}
var page_count = 1;
...
async  function ReviewScrapper(){
   ...
   //chrome.storage.local.get(['manual_scrap_user_id'], function(result) {
   // user_id = result.manual_scrap_user_id;


   var hasNextPage = true;
   var currentPage = parseInt(jQuery('.ml-current-page span').text());
   ...
   var mainPlan = jQuery('.fieldset-wrapper-metother span#dropdown-default-combo-label').text();
   var subPlanOriginal = jQuery('span.change-plan-text').text();
   var subPlan = subPlanOriginal.replace(/You chose\s*|\s*plan\./g, '');
   ...
   for (let i = 0; i < cards.length; i++) {
    ...
    const current = jQuery(cards[i]);
    var block_no = current.find('.text-index-front-card').text();
    const searchFields = mainPlan +'--'+ subPlan +'--'+ currentPage +'--'+block_no;
    
    var address2 = current.find('.name:last > span').text();
  //  var businessName = current.find('.name > span.business').text();

    var bussinessName = '';
    var bussinessCheckHMO = current.find('.cards-content > div:first-child span.dental-office-bold-unit:first-child').text();
    var bussinessCheckPDP = current.find('.name > span.business');
    if(bussinessCheckHMO == 'Dental office:'){
     var  bussinessName = current.find('.cards-content > div:first-child span.dental-office-bold-unit:first-child + span').text();
    }else if(bussinessCheckPDP.length){
        var bussinessName = bussinessCheckPDP.text();
    }
    let address1 = current.find('.name > span:not([class])').map(function () {return $(this).text(); }).get().join(', ');
    
    if(bussinessName != ''){
        var completeAddress = bussinessName + ', ' + address1;
    }else{
    ...
    const phone = current.find('.phone-details > span').text();
    const insurance = 'Metlife';
    const distance = current.find('.miles-distance > span').text().trim();
    const specialties = current.find('.specialties-details > span:nth-child(2)').text();
    

    current.find(".front-card-name-link span").click();

    await new Promise(resolve => setTimeout(resolve, 1600));
    
    const getGmail = jQuery('.cards-content .email-link span').text();
    const email = getGmail === '' ? 'nil' : getGmail;
    const getWebsite = jQuery('.website a >  span').text();
    const website = getWebsite === '' ? 'nil' : getWebsite;
    const id = jQuery('.turbo-card').attr('id');
    const name1 = jQuery('.turbo-card').attr('aria-label').trim()|| '';
    const name = name1.split(' ');
    const firstName = name[0]?.trim() || '';
    const secondName = name[1]?.trim() || '';
/*
...
*/
    ...
    // Wait before processing the next card
    await new Promise(resolve => setTimeout(resolve, 500));
    jQuery(".closebtn button span").click();


}


    const nextPageButton = jQuery('[title*="View page '+(currentPage + 1 )+'"]'); // Update with the correct selector for the next page button
    if (nextPageButton.length > 0) {
        console.log('Moving to the next page...');
        nextPageButton.find('span').click();
        ...
        hasNextPage = false; // Exit the loop if no next page button is found
    }
}
